...
do
  ...
  BENCHMARKCOMMAND='cd tamperproofbroadcast; git pull; cd ..; MULTICHAINHOST=$(getent hosts multichain-headless | cut -d" " -f1); python3 tamperproofbroadcast/tests/benchmarks/benchmark.py --benchmark-duration=600 --benchmark-testid=fotb-'$N_REPLICAS'-$(hostname) fotb --multichain-host=$MULTICHAINHOST --multichain-port=7208 --multichain-chainname=chain@$MULTICHAINHOST --fotb-startheight=888;'
  ...
  BENCHMARKCOMMAND='cd tamperproofbroadcast; git pull; cd ..; MULTICHAINHOST=$(getent hosts multichain-headless | cut -d" " -f1); python3 tamperproofbroadcast/tests/benchmarks/benchmark.py --benchmark-duration=600 --benchmark-testid=totb-'$N_REPLICAS'-$(hostname) totb --multichain-host=$MULTICHAINHOST --multichain-port=7208 --multichain-chainname=chain@$MULTICHAINHOST --totb-startheight=888;'
  cat $SCRIPTPATH/benchmark-deployment.yaml.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTYAML
  cat $SCRIPTPATH/benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTSCRIPT
  cat $SCRIPTPATH/delete-benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DELETEDEPLOYMENTSCRIPT
  ...
  BENCHMARKCOMMAND='cd tamperproofbroadcast; git pull; cd ..; MULTICHAINHOST=$(getent hosts multichain-headless | cut -d" " -f1); python3 tamperproofbroadcast/tests/benchmarks/benchmark.py --benchmark-duration=60 --benchmark-testid=htlltb-'$N_REPLICAS'-$(hostname) htlltb --multichain-host=$MULTICHAINHOST --multichain-port=7208 --multichain-chainname=chain@$MULTICHAINHOST --fotb-startheight=888 --etcd-host=etcd.default.svc.cluster.local --etcd-port=2379;'
  cat $SCRIPTPATH/benchmark-deployment.yaml.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTYAML
  cat $SCRIPTPATH/benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTSCRIPT
  cat $SCRIPTPATH/delete-benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DELETEDEPLOYMENTSCRIPT
  ...
  BENCHMARKCOMMAND='cd tamperproofbroadcast; git pull; cd ..; MULTICHAINHOST=$(getent hosts multichain-headless | cut -d" " -f1); python3 tamperproofbroadcast/tests/benchmarks/benchmark.py --benchmark-duration=60 --benchmark-testid=htlltbtest-'$N_REPLICAS'-$(hostname) htlltbtest --etcd-host=etcd.default.svc.cluster.local --etcd-port=2379;'
  cat $SCRIPTPATH/benchmark-deployment.yaml.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTYAML
  cat $SCRIPTPATH/benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DEPLOYMENTSCRIPT
  cat $SCRIPTPATH/delete-benchmark-deployment.sh.tmpl | envsubst > $SCRIPTPATH/tmp/$DELETEDEPLOYMENTSCRIPT
done
